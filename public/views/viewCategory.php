...
<div class="h-screen">
    <section class="w-[90%] lg:w-[80%] xl:w-[80%] h-[80%] mt-[5%] rounded-lg shadow-md space-y-4 max-h-fit bg-white mx-auto gap-x-7 gap-y-9 px-6 py-4 overflow-x-hidden dark:bg-slate-700">
        ...
        <div class="space-y-2">
            <?php $postsCategory = getPostsWhereCat($category["id"], -1, "desc"); foreach ($postsCategory as $postCategory): ?>
                <article class="w-full border-b border-spacing-0 h-16 m-0 px-3">
                    <div class="flex flex-row items-center justify-between gap-x-3">
                        <a class="2xl:w-2/3 xl:w-2/3 lg:w-1/2" href="index.php?page=post&ref=<?= $postCategory["reference"]?>">
                            <div class="group">
                                ...
                                <p class="hidden lg:block text-sm text-gray-500 group-hover:text-gray-400 transition-all duration-75 dark:text-slate-300"><?= substr($postCategory["description"], 0, 40) ?>...</p>
                            </div>
                        </a>
                        <div class="flex flex-row w-1/2 xl:w-1/3 2xl:w-1/3 items-center gap-x-8">
                            <div class="hidden lg:flex xl:flex flex-col w-fit h-fit items-center gap-x-2">
