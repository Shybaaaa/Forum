...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["updatePost"])) {
    if (!isset($_POST["title"]) && !isset($_POST["description"]) && !isset($_POST["category"])) {
        ...
        addPost($_POST["title"], $_POST["description"], $_POST["category"], uploadImage($_FILES["image"]), $_SESSION["user"]["id"]);
    }
}
    ...
    <div class="overflow-x-auto h-full flex flex-col justify-between">
        <div>
            <div class="w-full m-3">
                <h3 class="text-lg font-semibold border-b-2 border-spacing-y-3 w-full border-separate border-gray-200 dark:text-slate-200">Création d'un post</h3>
            </div>
            <form action="" method="post" class="space-y-3" enctype="multipart/form-data">
                <div class="col-span-full">
                    <div>
                        ...
                        <div class="mt-2 flex items-center gap-x-3">
                            <input class="block w-[90%] mx-auto text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" name="image" accept="image/jpeg, image/png," aria-describedby="post_image" id="image" type="file">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-row flex items-center justify-around">
                    ...
                    <input type="text" name="title" id="title" autocomplete="title" class="rounded-lg mx-auto border-gray-300 w-1/2 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-400 focus:ring-opacity-50 dark:bg-slate-600 dark:text-slate-300" value="">
                </div>
                <div class="border-b border-separate w-1/2 mx-auto"></div>
                ...
                <div class="w-full">
                    ...
                    <textarea name="description" id="description" rows="15" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-400 focus:ring-opacity-50 dark:bg-gray-600 dark:text-slate-300"></textarea>
                </div>
                <div>
                    <input type="submit" value="Créer" name="updatePost" id="updatePost" class="w-full py-2 px-4 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-500 transition-all">
                </div>
            </form>
        </div>
    </div>
